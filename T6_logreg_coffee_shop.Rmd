...
confusionMatrix(data = prediction$prediction, 
...
observations <- as.numeric(paste(test_latte_donut$LatteDonut))
predictions <- as.numeric(paste(prediction$prediction))
data <- data.frame(cbind(observations, predictions))
...
```{r}
coffee_donut_prediction <- predict(late_donut_lr_model,
                                   newdata = lattes_new,
                                   type = "response")
